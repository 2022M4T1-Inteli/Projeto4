...
// Funcao de configuracao do MQTT
void setupMQTT() {
...
void reconnect() {
  ...
  while (!mqttClient.connected()) {
    ...
    if (mqttClient.connect(clientId.c_str(), "Inteli-iot", "NDD7_@hNgHY2vRN")) { // Informacoes do nosso cliente no servidor MQTT
...
void setup() {
  ...
  setupMQTT(); // Chamando a funcao de configuracao do servidor MQTT
}
void loop() {
  ...
  mqttClient.loop(); // Faz com que o servidor MQTT continue rodando e nao se desconecte
  long now = millis(); // Conta quantos milisegundos o ESP esta em execucao
  long previous_time = 0;

  // Pegando o endreco MAC do ESP e definindo ele como uma variavel
  String rede = WiFi.macAddress();
  //search();
 // delay(1800000);
}
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  for (int i = 0; i < length; i++) {
    Serial.print((char)message[i]); // Mensagens retornadas do servidor sao printadas no console.
  }
} 
